...
public class Serial extends CordovaPlugin {
	...
	private static final String ACTION_PURGE_BUFFERS = "purgeHwBuffers";
	private static final String ACTION_READ_DEPLAY = "registerReadDelay";
	...
	public boolean execute(String action, JSONArray args, final CallbackContext callbackContext) throws JSONException {
		...
		else if (ACTION_READ_DEPLAY.equals(action)) {
			registerReadDelay(callbackContext);
			return true;
		}
		// purge hardware buffers
		else if (ACTION_PURGE_BUFFERS.equals(action)) {
			purgeHwBuffers(callbackContext);
			return true;
		}
			...
			public void run() {
				...
				try {
					if (port != null) {
						...
						callbackContext.error("Invalid port number");
					}
				} catch (Exception e) {
					callbackContext.error(e.getMessage());
				}
				...
				try {
					Thread.sleep(100); // Add a delay to allow any buffered data to be sent
	...
	private void purgeHwBuffers(final CallbackContext callbackContext) {
		cordova.getThreadPool().execute(new Runnable() {
			public void run() {
				if (port == null) {
					callbackContext.error("Closing a closed port.");
				} else {
					try {
						port.purgeHwBuffers(true, true);
						callbackContext.success();
	...
	private void purgeHwBuffers2(final CallbackContext callbackContext) {
		cordova.getThreadPool().execute(new Runnable() {
			public void run() {
				clearBuffer();
				String str = "1|";
				if (port == null) {
					str += "port - null|";
					//callbackContext.error("Purging buffers on a closed port.");
				} else {
					str += "port|";
					//callbackContext.success("Purging buffers cleared");
					/*
					try {
						port.purgeHwBuffers(true, true);
						callbackContext.success("Purging buffers cleared");
				...
				if (mSerialIoManager == null) {
					str += "mSerialIoManager - null|";
				} else {
					str += "mSerialIoManager|";
				}
	...
	/**
     * Update the received data and send it to the callback.
     */
    private synchronized void updateReceivedData() {
        if (runBroadcast && newDataReceived && readCallback != null && receivedDataBuffer.size() > 0) {
            JSONArray dataArray = new JSONArray();
            for (byte[] data : receivedDataBuffer) {
                dataArray.put(Base64.encodeToString(data, Base64.DEFAULT));
            }
            receivedDataBuffer.clear();
            newDataReceived = false;
            PluginResult result = new PluginResult(PluginResult.Status.OK, dataArray);
            result.setKeepCallback(true);
            readCallback.sendPluginResult(result);
        }
    }

	private void registerReadDelay(final CallbackContext callbackContext) {
		cordova.getThreadPool().execute(new Runnable() {
			public void run() {
				runBroadcast = true;
			}
	...
	private void registerReadCallback(final CallbackContext callbackContext) {
		...
		cordova.getThreadPool().execute(new Runnable() {
			public void run() {
				...
				/*if (newDataReceived && readCallback != null && receivedDataBuffer.size() > 0) {
					JSONArray dataArray = new JSONArray();
					for (byte[] data : receivedDataBuffer) {
						dataArray.put(Base64.encodeToString(data, Base64.DEFAULT));
					}
					...
					PluginResult pluginResult = new PluginResult(PluginResult.Status.OK, dataArray);
					pluginResult.setKeepCallback(true);
					callbackContext.sendPluginResult(pluginResult);
				}*/
				
        		//updateReceivedData(); // Send any buffered data to the callback
